...
Anne_PCA_plot<- function(pca_data, my_prefix, my_title, ratioaxes="true" ) {	
	...
	pca_summary <- summary(experiments.pca)
	# write pca_summary data
	filename <- paste(experiment, my_prefix, "summary.txt", sep=".")
	write.table(t(round_df(pca_summary$importance,2)), file=filename , quote=F, sep="\t", col.names=NA)	
	...
	if (ratioaxes == "true") {
		...
		# write rotation axis
		filename <- paste(experiment, my_prefix, "rotation.txt", sep=".")
		write.table(round_df(rotation_axis,2), file=filename , quote=F, sep="\t", col.names=NA)	
	...
	write.table(round_df(my_pca_data,2), file=filename , quote=F, sep="\t", col.names=NA)	
}
...
Class_signal_plots <- function(my_data, my_description) {	
	...
	for (i in levels(my_class_genes$Group)) {
		...
		if (nrow(my_subset_table)>1) {   # do not draw tables less then 2 genes
			...
			print(my_title)
			p1 <- qplot(x=Var2, y=log(value,2), data=m1, group=Var1,colour = factor(Var1), main = my_title, xlab="Factors", ylab="Log2(Signal)", geom = "line") + theme_g2d_plot
			# , ylim=c(min_y,max_y)
			print(p1)
			dev.off()
		}
...
SubClass_correlation_plots <- function(my_data, my_description) {	
	...
	for (i in levels(my_class_genes$Group)) {
		...
		if (nrow(my_subset_table)>1) {   # do not draw tables with less then 2 genes
			...
			my_title=paste(my_description," = ",i)
			p1 <- qplot(x=Var1, y=Var2, data=as.data.frame(m1),main = my_title, xlab="Class genes", ylab="Class genes", fill=value, geom="tile") + theme_g2d_plot_cor + scale_fill_gradient2(limits=c(-1, 1))
			print(p1)
			dev.off()
		}
	}
	...
	for (i in 1:nclusters) {
		...
		if (nrow(cluster2)>0) {
			...
			if( mode(my_data) == "numeric") { 
				...
				p1 = qplot(x=Var2, y=value, data=m1, group=Var1, main = p1_title, colour = factor(Var1), xlab="Experiment", ylab=y_label, geom = "line",  ylim=c(min_y,max_y)) + theme_kmeansplot + scale_colour_hue(name = "GeneID")
			} else {
				...
				p1 = qplot(x=Var2, y=y_values, data=m1, group=Var1, main = p1_title, colour = factor(Var1), xlab="Experiment", ylab=y_label, geom = "line",  ylim=c(min_y,max_y)) + theme_kmeansplot + scale_colour_hue(name = "GeneID")
			}	
